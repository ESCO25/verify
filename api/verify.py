...
def handler(request):
    h = {"Access-Control-Allow-Origin":"*","Access-Control-Allow-Methods":"POST, OPTIONS","Access-Control-Allow-Headers":"Content-Type","Content-Type":"application/json; charset=utf-8"}
    ...
    try:
        ...
        for t,v in [("ip",ip),("device",device_id),("canvas",data.get("canvasFp",""))]:
            if db["bans"].find_one({"type":t,"value":v,"guild_id":guild_id}):
                db["blocked_attempts"].insert_one({"discord_id":discord_id,"guild_id":guild_id,"ip":ip,"device_id":device_id,"reason":f"banned_{t}","timestamp":datetime.now(timezone.utc)})
                return (json.dumps({"success":False,"banned":True,"message":"جهازك أو شبكتك محظورة"},ensure_ascii=False), 403, h)
        db["members"].update_one({"discord_id":discord_id,"guild_id":guild_id},{"$set":{"discord_id":discord_id,"guild_id":guild_id,"ip":ip,"device_id":device_id,"canvas_fp":data.get("canvasFp",""),"webgl":data.get("webglRenderer",""),"user_agent":data.get("userAgent",""),"screen":f"{data.get('screenW')}x{data.get('screenH')}","timezone":data.get("timezone",""),"verified_at":datetime.now(timezone.utc)}},upsert=True)
        db["pending_verifications"].delete_one({"token":token})
        db["verification_queue"].insert_one({"discord_id":discord_id,"guild_id":guild_id,"action":"approve","timestamp":datetime.now(timezone.utc)})
